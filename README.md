# 🤖 Gemini 학습
특정 문서(`manual.txt`)와 위키백과를 참고하여 질문에 답변하는 챗봇입니다. 검색 증강 생성(RAG) 아키텍처를 사용하여 정확하고 신뢰성 있는 답변을 제공합니다.
...
- **다중 소스 지원**: 로컬 파일과 웹사이트 URL을 모두 학습하여 답변을 생성합니다.
- **복합 질문 처리**: 질문과 관련된 여러 개의 문서 덩어리를 찾아 답변을 생성하여, 복합적인 질문에도 정확하게 응답합니다.
